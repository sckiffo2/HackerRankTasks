...
public class SteamAPI {
	private List<Employee> emps = new ArrayList<>();
	{
		...
		emps.add(new Employee("Jane",    "Smith",   523,  40, Position.MANAGER));
		emps.add(new Employee("Jury",    "Gagarin", 6423, 26, Position.MANAGER));
		emps.add(new Employee("Jack",    "London",  5543, 53, Position.WORKER));
		emps.add(new Employee("Eric",    "Jackson", 2534, 22, Position.WORKER));
		emps.add(new Employee("Andrew",  "Bosh",    3456, 44, Position.WORKER));
		emps.add(new Employee("Joe",     "Smith",   723,  30, Position.MANAGER));
		emps.add(new Employee("Jack",    "Gagarin", 7423, 35, Position.MANAGER));
		emps.add(new Employee("Jane",    "London",  7543, 42, Position.WORKER));
		emps.add(new Employee("Mike",    "Jackson", 7534, 31, Position.WORKER));
		emps.add(new Employee("Jack",    "Bosh",    7456, 54, Position.WORKER));
		emps.add(new Employee("Mark",    "Smith",   123,  41, Position.MANAGER));
		emps.add(new Employee("Jane",    "Gagarin", 1423, 28, Position.MANAGER));
		emps.add(new Employee("Sam",     "London",  1543, 52, Position.WORKER));
		emps.add(new Employee("Jack",    "Jackson", 1534, 27, Position.WORKER));
		emps.add(new Employee("Eric",    "Bosh",    1456, 32, Position.WORKER));
	}

	private List<Department> deps = new ArrayList<>();
	{
		deps.add(new Department(1, 0, "Head"));
		deps.add(new Department(2, 1, "West"));
		deps.add(new Department(3, 1, "East"));
		deps.add(new Department(4, 2, "Germany"));
		deps.add(new Department(5, 2, "France"));
		deps.add(new Department(6, 3, "China"));
		deps.add(new Department(7, 3, "Japan"));
	}


	@Test
	public void creation() throws IOException {
		Stream<String> lines = Files.lines(Paths.get("./src/examples/Streams/some.txt"));
		...
		IntStream intStream = IntStream.rangeClosed(10, 100); //10 .. 100
